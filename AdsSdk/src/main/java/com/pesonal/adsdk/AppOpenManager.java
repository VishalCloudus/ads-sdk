...
import static com.pesonal.adsdk.ADS_SplashActivity.splashopen;
...
public class AppOpenManager implements Application.ActivityLifecycleCallbacks, LifecycleObserver {
    ...
    public static boolean resenetopen = false;
    private final Activity myApplication;
    private Activity currentActivity;
    AppPreferenceVpn appPreferenceVpn;

    public AppOpenManager(Activity myApplication) {
        this.myApplication = myApplication;
        appPreferenceVpn = new AppPreferenceVpn(myApplication);
    ...
    public void showAdIfAvailable(final splshADlistner listner) {
        if (AppManage.vpn_flag.equalsIgnoreCase("1") && appPreferenceVpn != null) {
            if (appPreferenceVpn.getIsVpnConnected() && !appPreferenceVpn.getIsCountrySame()) {
            ...
            } else if (appPreferenceVpn.getIsCountrySame()) {
                if (!isShowingAd && isAdAvailable()) {
                    Log.d(LOG_TAG, "Will show ad.");
                    FullScreenContentCallback fullScreenContentCallback =
                            new FullScreenContentCallback() {
    ...
    public void fetchAd(final splshADlistner listner) {
        if (!isShowingAd && isAdAvailable()
                && !appPreferenceVpn.getIsVpnConnected()
                && appPreferenceVpn.getIsCountrySame()) {
            return;
        }
    ...
    public void fetchAd() {
        if (isAdAvailable() && !appPreferenceVpn.getIsVpnConnected()
                && appPreferenceVpn.getIsCountrySame()) {
            return;
        }
